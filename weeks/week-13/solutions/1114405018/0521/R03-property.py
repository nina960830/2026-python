...
class Student:
    def __init__(self, name, grade):
        # 注意：這裡指派 self.grade 會自動走到下面的 setter
        self.name = name
    ...
    def grade(self):
        ...
        # 真正的資料放在 _grade，避免和 property 名稱自己撞名
...
class Circle:
    def __init__(self, radius):
        # 半徑是基本資料，面積與直徑都可以由它計算出來
        self.radius = radius

    @property
    def area(self):
        """面積是計算出來的，不該被直接設定"""
        # 每次讀取都即時計算，確保半徑變動後結果也會同步更新
        import math

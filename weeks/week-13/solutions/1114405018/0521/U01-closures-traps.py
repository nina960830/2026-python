...
# 等迴圈跑完，i 已經變成最後一個數字，所以所有 lambda 都讀到同一個值。

print("\n=== 陷阱 2：閉包延遲綁定 ===")
funcs = []
for i in range(5):
    # 這裡的 lambda 沒有把 i 固定下來，因此會發生延遲綁定
...
# 閉包預設只能「讀取」外層變數；
# 要修改外層變數，必須用 nonlocal 宣告，告訴 Python 這不是新區域變數。
...
def make_counter(start=0):
    ...
    def counter():
        # nonlocal 表示要操作外層的 count，不是在內層新建一個同名變數
